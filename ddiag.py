...
db = web.database(dbn='sqlite', db='mltdb')
...
urls = ('/','index', 
		...
		'/getbill/(.+)','getbill',
		'/getbilldiscount/(.+)','getbilldiscount',
        '/delbill/(.+)','delbill',
...
class register:
    def POST(self):
        ...
        db.insert("LOGS",message = msg)
		#,"_parent"
        #raise web.seeother("/testreq","innerframe")
        return web.seeother("/home","_parent")
...
class getpatientdata:
    def GET(self,pcode):
        pdata = db.query("SELECT PCODE,PNAME, PPHONE, ADDRESS, REF_DR_NAME, AGE, SEX  FROM PATIENT WHERE PCODE = '%s' ORDER BY PCODE DESC" % pcode )
...
class getallpatientdata:
    def GET(self):
        pdata = db.query("SELECT PCODE as recid, PCODE,PNAME, PPHONE, ADDRESS, REF_DR_NAME, AGE, SEX  FROM PATIENT ORDER BY PCODE DESC")
        #print json.dumps(pdata[0])
        pdata = list(pdata)
...
class testreq:
    def GET(self):
        ...
        opts = db.query("SELECT s.TESTCODE,s.STESTNAME,s.SUBTSTCODE,AGAIN,amt FROM SUBTEST s ORDER BY s.STESTNAME ASC")
        opts = list(opts)
...
class saverequest:
    def POST(self):
        ...
        if int(treq["billid"]) == 0:
            billno = db.insert('BILLS',pcode=treq["pcode"],billdate=treq["billdate"], billdata = treq["billdata"],billenteredby= d["username"], valuesaved='N',discount=treq["discount"] )
            msg = "Bill No:"+ str(billno) +" Entered by "  + d["username"]
            db.insert("LOGS",message = msg)
        else:
            billno = treq["billid"]
            tvid   = db.update( 'BILLS',where='billid = $billno', vars = locals(),pcode=treq["pcode"],billdate=treq["billdate"], billdata = treq["billdata"],billenteredby= d["username"], valuesaved='N',discount=treq["discount"])
...
class savepatientdata:
    def POST(self):
        ...
        psex  = preq["SEX"]
        pname = preq["PNAME"]
        paddress = preq["ADDRESS"]
        pphone = preq["PPHONE"]
        pref_dr_name= preq["REF_DR_NAME"]
        pvid = db.update('PATIENT',where='PCODE = $pcode',vars=locals(),PNAME=pname,AGE=page,SEX=psex,address=paddress,PPHONE=pphone,REF_DR_NAME=pref_dr_name)
		
        msg = "Patient Data For No:"+ str(pcode) +" Updated by "  + d["username"]    
...
class getbillsnotfilled:
    def GET(self):
        ...
        sqlx = """
            SELECT billid as recid, DATE_FORMAT(b.billdate,'%Y-%m-%d') as billdate,b.billid,b.pcode,b.discount, p.pname,p.address, p.SEX, p.AGE, p.REF_DR_NAME 
            FROM  BILLS  b
...
class showbillform:
    def GET(self,bid):
        ...
        if bdata[0]['CNT'] != 0:
            ...
            sqlx = """
				  SELECT billdata,discount FROM BILLS WHERE billid = %s
			""" % ( bid )
            bdata = db.query(sqlx)
            bdata = list(bdata)
            objs =  bdata[0] 
            ...
            sqlx = """
				  SELECT CASE s.AGAIN
						 ...
						 END as NOR_VALUE,
						 CASE s.AGAIN
						     WHEN 'Y' THEN IFNULL(ss.UNIT,'')
                             WHEN 'N' THEN IFNULL(s.UNIT,'')
                             ELSE ' ' 
					     END as UNIT,
                    	 ' ' as ACT_VAL
				  FROM SUBTEST s 
...
class gettestname:
    def GET(self,tcode):
        sqlx = """ SELECT TESTNAME FROM MAINTEST m INNER JOIN SUBTEST t on t.TESTCODE = m.TESTCODE
		           WHERE  t.TESTCODE = %s
                   GROUP BY t.TESTCODE
        """ % ( tcode )
        tname = db.query(sqlx)
...
class addconftests:
    def GET(self):
        d = session.get('userdata',{"loggedin":False,"username":""})
        categs = db.query("SELECT testcode as recid, testcode,testname FROM MAINTEST");
        categs = list(categs)
        d["categories"] = categs
              
        subtests = db.query("SELECT TESTCODE as recid,STESTNAME,SDESCR,NOR_VALUE,SUBTSTCODE,AGAIN,amt FROM SUBTEST");
...
class getbilldiscount:
    def GET(self,bid):
        d = session.get('userdata',{"loggedin":False,"username":""})
        sqlx = """
				  SELECT CONVERT(discount,CHAR(11)) as discount FROM BILLS WHERE billid = %s
			""" % ( bid )
        bdata = db.query(sqlx)
        bdata = list(bdata)
        ...
        objs =  bdata[0]
        print objs
        objslist = json.loads( objs["discount"] )
        #d["existingbill"] = True
        #print objslist
        return json.dumps(objslist)
...
class reportgen:
    def POST(self):
        ...
        sqlx = """
                    ...
                    b.billenteredby as billenteredby, b.discount as discount 
                    FROM BILLS b
                    INNER JOIN PATIENT p on p.pcode = b.pcode
...
class getlastbillno:
    def GET(self,bid):
        sqlx = """
                  SELECT MAX(billid) as billno FROM BILLS 
               """
        if not int(bid) == 0:
            sqlx += " WHERE billid = %d" % ( int(bid) )
        bdata = db.query(sqlx)
        bdata = list( bdata )
        if bdata[0]["billno"]==  None:
            bdata[0]["billno"] = 0
		   
		
        return json.dumps( bdata[0] )		
def default(o):
